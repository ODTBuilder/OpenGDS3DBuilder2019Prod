...
public class Shp2obj {
	...
	public static void main(String[] args) throws Exception {
		...
		File buildingFile = new File("D:\\data\\TL_building_clipped.shp");
		File woodlandFile = new File("D:\\data\\TL_woodland_clipped.shp");
		Filter filter = Filter.INCLUDE; // ECQL.toFilter("BBOX(THE_GEOM, 10,20,30,40)")

		try (BufferedWriter writer = new BufferedWriter(
				new OutputStreamWriter(new FileOutputStream("D:\\data\\utput.obj"), "utf-8"))) {
			...
			FeatureCollection<SimpleFeatureType, SimpleFeature> buildingCollection = getFeatureCollectionFromFileWithFilter(
					buildingFile, filter);
			FeatureCollection<SimpleFeatureType, SimpleFeature> woodlandCollection = getFeatureCollectionFromFileWithFilter(
					woodlandFile, filter);
