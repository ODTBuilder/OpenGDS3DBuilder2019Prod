...
public class GLProgram {
    ...
    public Map<String, Integer> fetchAttributeLocations(int program) {
        ...
        int n = GLES20.glGetProgrami(program, GLES20.GL_ACTIVE_ATTRIBUTES);
        int strLen = GLES20.glGetProgrami(program, GLES20.GL_ACTIVE_ATTRIBUTE_MAX_LENGTH);
        for (int i = 0; i < n; i++) {
            
            String name = "";//GLES20.glGetActiveAttrib(program, i, strLen);
            attriutes.put(name, GLES20.glGetAttribLocation(program, name));
        }
        return attriutes;
    }
