...
public class Geometry implements IGeometry {
    ...
    public final List<Face3> faces;
    public final List<List<List<Vector2d>>> faceVertexUvs;

    public final List<Double> lineDistances;
    ...
    public Geometry fromBufferGeometry(BufferGeometry geometry) {
        ...
        for (int i = 0, j = 0; i < positions.size(); i += 3, j += 2) {
            ...
            this.vertices.add(new Vector3d(positions.get(i).doubleValue(), positions.get(i + 1).doubleValue(), positions.get(i + 2).doubleValue()));

            if (normals != null) {
                tempNormals.add(new Vector3d(normals.get(i).doubleValue(), normals.get(i + 1).doubleValue(), normals.get(i + 2).doubleValue()));
            }

            if (colors != null) {
                this.colors.add(new Color(colors.get(i).floatValue(), colors.get(i + 1).floatValue(), colors.get(i + 2).floatValue()));
            }
        }
            ...
            if (groups.size() > 0) {
                for (int i = 0; i < groups.size(); i++) {
                    ...
                    for (int j = start, j1 = start + count; j < j1; j += 3) {
                        addFace(indices.get(j).intValue(), indices.get(j + 1).intValue(), indices.get(j + 2).intValue(), group.materialIndex, normals, tempNormals, colors);
                    }
                }
            } else {
                for (int i = 0; i < indices.size(); i += 3) {
                    addFace(indices.get(i).intValue(), indices.get(i + 1).intValue(), indices.get(i + 2).intValue(), null, null, null, null);
                }
            }
        } else {
    ...
    private void addFace(int a, int b, int c, Integer materialIndex, List<Number> normals, List<Vector3d> tempNormals, List<Number> colors) {
        List<Vector3d> vertexNormals = normals != null ? new ArrayList<>(Arrays.asList(new Vector3d[]{tempNormals.get(a).copy(), tempNormals.get(b).copy(), tempNormals.get(c).copy()})) : new ArrayList<>();
        List<Color> vertexColors = colors != null ? new ArrayList<>(Arrays.asList(new Color[]{this.colors.get(a).copy(), this.colors.get(b).copy(), this.colors.get(c).copy()})) : new ArrayList<>();
    ...
    public void merge(Geometry geometry, Matrix4d matrix, Integer materialIndexOffset) {
        ...
        for (int i = 0, il = faces2.size(); i < il; i++) {
            ...
            faceCopy = new Face3(face.a + vertexOffset, face.b + vertexOffset, face.c + vertexOffset, new Vector3d().copy(face.normal));
    ...
    public int mergeVertices() {
        Map<String, Integer> verticesMap = new HashMap<>(); // Hashmap for looking up vertices by position coordinates (and making sure they are unique)
